...
public class Rsa2008Tests
{
    [Theory]
    [InlineData(DesignTestData.Correct)]
    [InlineData(DesignTestData.GenericSiblings)]
    public async Task GivenCorrect_WhenAnalyze_ThenNoDiagnosticsReported(string source)
    {
        ...
        result
           .AnalyzerResults
           .Should()
           .NotContain(pair => pair.Value.Diagnostics.Any(diagnostic => diagnostic.Id == RSA2008.Id));
    }

    [Theory]
    [InlineData(DesignTestData.MultipleTypes)]
    public async Task GivenIncorrect_WhenAnalyze_ThenDiagnosticsReported(string source)
    {
        ...
        // Then
        result
           .AnalyzerResults[typeof(Rsa2008)]
           .Diagnostics
           .Should()
           .NotBeEmpty()
           .And
           .OnlyContain(diagnostic => diagnostic.Id == RSA2008.Id);
    }

    [Theory]
    [InlineData(nameof(DesignTestData.Correct), DesignTestData.Correct)]
    [InlineData(nameof(DesignTestData.MultipleTypes), DesignTestData.MultipleTypes)]
    [InlineData(nameof(DesignTestData.GenericSiblings), DesignTestData.GenericSiblings)]
