...
public class Rsa2006Tests
{
    ...
    public async Task GivenCorrect_WhenAnalyze_ThenNoDiagnosticsReported(string source)
    {
        ...
        result
           .AnalyzerResults
           .Should()
           .NotContain(pair => pair.Value.Diagnostics.Any(diagnostic => diagnostic.Id == RSA2006.Id));
    }

    [Theory]
    [InlineData(DesignTestData.FieldBeforeConstant)]
    public async Task GivenIncorrect_WhenAnalyze_ThenDiagnosticsReported(string source)
    {
        ...
        // Then
        result
           .AnalyzerResults[typeof(Rsa2006)]
           .Diagnostics
           .Should()
           .ContainSingle()
           .Which
    ...
    {
        // Given
        var sources = DesignTestData.Ordering.Where(source => source != DesignTestData.FieldBeforeConstant);
        ...
        foreach (var source in sources)
        {
            ...
            result
               ...
               .Diagnostics
               .Should()
               .NotContain(diagnostic => diagnostic.Id == RSA2006.Id, "only one rule owns a rank component");
        }
    }

    [Theory]
    [InlineData(nameof(DesignTestData.Correct), DesignTestData.Correct)]
    [InlineData(nameof(DesignTestData.FieldBeforeConstant), DesignTestData.FieldBeforeConstant)]
    ...
    public async Task GivenOtherShapes_WhenAnalyze_ThenNoDiagnosticsReported()
    {
        ...
        foreach (var source in DesignTestData.CorrectShapes)
        {
            ...
            result
               ...
               .Diagnostics
               .Should()
               .NotContain(diagnostic => diagnostic.Id == RSA2006.Id);
        }
    }
}
