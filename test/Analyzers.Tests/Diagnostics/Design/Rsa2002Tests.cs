...
public class Rsa2002Tests
{
    ...
    public async Task GivenCorrect_WhenAnalyze_ThenNoDiagnosticsReported(string source)
    {
        ...
        result
           .AnalyzerResults
           .Should()
           .NotContain(pair => pair.Value.Diagnostics.Any(diagnostic => diagnostic.Id == RSA2002.Id));
    }

    [Theory]
    [InlineData(DesignTestData.PrivateBeforeNonPrivate)]
    ...
    {
        // Given
        var sources = DesignTestData.Ordering.Where(source => source != DesignTestData.PrivateBeforeNonPrivate);
        ...
        foreach (var source in sources)
        {
            ...
            result
               ...
               .Diagnostics
               .Should()
               .NotContain(diagnostic => diagnostic.Id == RSA2002.Id, "only one rule owns a rank component");
        }
    }

    [Theory]
    [InlineData(nameof(DesignTestData.Correct), DesignTestData.Correct)]
    [InlineData(nameof(DesignTestData.PrivateBeforeNonPrivate), DesignTestData.PrivateBeforeNonPrivate)]
