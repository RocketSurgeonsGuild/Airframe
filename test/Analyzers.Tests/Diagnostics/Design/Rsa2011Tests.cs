...
public class Rsa2011Tests
{
    ...
    public async Task GivenCorrect_WhenAnalyze_ThenNoDiagnosticsReported(string source)
    {
        ...
        result
           .AnalyzerResults
           .Should()
           .NotContain(pair => pair.Value.Diagnostics.Any(diagnostic => diagnostic.Id == RSA2011.Id));
    }

    [Theory]
    [InlineData(DesignTestData.ImplicitAccessibility)]
    public async Task GivenIncorrect_WhenAnalyze_ThenDiagnosticsReported(string source)
    {
        ...
        // Then
        result
           .AnalyzerResults[typeof(Rsa2011)]
           .Diagnostics
           .Should()
           .NotBeEmpty()
           .And
           .OnlyContain(diagnostic => diagnostic.Id == RSA2011.Id);
    }

    [Theory]
    [InlineData(nameof(DesignTestData.Correct), DesignTestData.Correct)]
    [InlineData(nameof(DesignTestData.ImplicitAccessibility), DesignTestData.ImplicitAccessibility)]
